...
import { describeWorkScopeTags, getWorkScopeTags, parseWorkScopeTags } from "@/lib/skillSetScopes";
import {
  AppConfig,
  EffectiveSkillSet,
  SkillSetAssignment,
...
export function ProjectProfile() {
  ...
  const scopes = useMemo(
    () => [...new Set(projectAssignments
      .filter((assignment) => assignment.role === "work_scope_overlay")
      .flatMap((assignment) => getWorkScopeTags(assignment)))],
    [projectAssignments],
  );
  ...
  const resolve = () => void (async () => {
    ...
    try {
      setEffectiveSet(await invoke<EffectiveSkillSet>("resolve_effective_skill_set", {
        request: { project_id: projectId || null, work_scope_tags: parseWorkScopeTags(workScope) },
      }));
    } catch (resolveError) {
  ...
  const createContextDraft = () => void (async () => {
    ...
    try {
      ...
      const scopeLabel = describeWorkScopeTags(effectiveSet);
      const catalog = await invoke<SkillSetStore>("create_skill_set_blueprint", {
        request: {
          name: `${projectName} ${scopeLabel} draft`,
          description: `Drafted from the ${projectName} project profile for ${scopeLabel}. Review the purpose, requirements, and members before releasing.`,
  ...
  return (
    <div className="h-full overflow-auto px-6 py-5">
      ...
      />
      <p className="mt-1 text-sm text-muted-foreground">Choose a project and current work scope tags before changing provider bindings. Priority controls the explanation order when several assignments apply.</p>
