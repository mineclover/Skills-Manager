...
export function SkillSets() {
  ...
  const applyActivation = (assignmentId: string) => void (async () => {
    setBusy(true); setError(null);
    try { const nextPlan = await invoke<SkillSetActivationPlan>("preview_skill_set_activation", { assignmentId }); setPlan(nextPlan); const confirmed = !nextPlan.requires_shared_root_confirmation || window.confirm(`This activation can affect shared provider roots: ${nextPlan.shared_impacts.map((impact) => impact.display_name).join(", ")}. Continue?`); if (!confirmed) return; const result = await invoke<SkillSetActivationApplyResult>("apply_skill_set_activation", { assignmentId, confirmSharedRoot: confirmed }); setPlan(result.plan); setActivationRuns((current) => [{ id: result.activation_run_id, assignment_id: result.plan.assignment_id, release_id: result.plan.release_id, project_id: result.plan.project_id, work_scope: result.plan.work_scope, applied_count: result.applied_count, skipped_count: result.skipped_count, failed_count: result.failed_count, provider_outcomes: result.provider_outcomes, created_at: Math.floor(Date.now() / 1000) }, ...current]); await load(); }
