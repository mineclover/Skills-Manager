...
import { describeWorkScopeTags, parseWorkScopeTags } from "@/lib/skillSetScopes";
import {
  AppConfig,
  Skill,
  SkillSetAssignment,
...
export function SkillSets() {
  ...
  const [assignmentRole, setAssignmentRole] = useState<SkillSetAssignment["role"]>("recommended");
  ...
  const createBlueprint = () => void updateStore(async () => {
    const catalog = await invoke<SkillSetStore>("create_skill_set_blueprint", {
      request: { name, description, skill_ids: [...memberIds], member_scope_policies: memberScopePolicies },
    });
    setName("");
    setDescription("");
    setMemberIds(new Set());
    setMemberScopePolicies({});
    return catalog;
  });
  ...
  const assignRelease = (release: SkillSetRelease) => void updateStore(() =>
    invoke<SkillSetStore>("assign_skill_set_release", {
      request: {
        ...
        work_scope_tags: parseWorkScopeTags(assignmentScope),
        role: assignmentRole,
  ...
  const applyActivation = (assignmentId: string) => void (async () => {
    setBusy(true); setError(null);
    try { const nextPlan = await invoke<SkillSetActivationPlan>("preview_skill_set_activation", { assignmentId }); setPlan(nextPlan); const confirmed = !nextPlan.requires_shared_root_confirmation || window.confirm(`This activation can affect shared provider roots: ${formatSkillBindingImpacts(nextPlan.shared_impacts)}. Continue?`); if (!confirmed) return; const result = await invoke<SkillSetActivationApplyResult>("apply_skill_set_activation", { assignmentId, confirmSharedRoot: nextPlan.requires_shared_root_confirmation && confirmed }); setPlan(result.plan); setActivationRuns((current) => [{ id: result.activation_run_id, assignment_id: result.plan.assignment_id, release_id: result.plan.release_id, project_id: result.plan.project_id, work_scope: result.plan.work_scope, applied_count: result.applied_count, skipped_count: result.skipped_count, failed_count: result.failed_count, provider_outcomes: result.provider_outcomes, created_at: Math.floor(Date.now() / 1000) }, ...current]); await load(); }
    catch (applyError) { setError(String(applyError)); }
    finally { setBusy(false); }
  })();

  const inspectDrift = (assignmentId: string) => void (async () => {
    setBusy(true); setError(null);
    try { setDrift(await invoke<SkillSetDriftReport>("get_skill_set_drift", { assignmentId })); }
    catch (driftError) { setError(String(driftError)); }
    finally { setBusy(false); }
  })();

  const loadActivationHistory = (releaseId: string) => void (async () => {
    setBusy(true); setError(null);
  ...
  const resolveEffectiveSet = () => void (async () => {
    setBusy(true); setError(null);
    try { setEffectiveSet(await invoke<EffectiveSkillSet>("resolve_effective_skill_set", { request: { project_id: assignmentProjectId || null, work_scope_tags: parseWorkScopeTags(assignmentScope) } })); }
      ...
      <section className="mt-5 grid gap-5 xl:grid-cols-[minmax(320px,0.9fr)_minmax(380px,1.1fr)]">
            ...
            <div className="mt-1 max-h-64 overflow-auto rounded-md border border-border p-2">
              {[...skillsById.values()].map((skill) => (
                <label key={skill.id} className="flex cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-sm hover:bg-muted">
                  ...
                  {memberIds.has(skill.id) && <select value={memberScopePolicies[skill.id] ?? "project_then_global"} onChange={(event) => setMemberScopePolicies((current) => ({ ...current, [skill.id]: event.target.value as "global" | "project" | "project_then_global" | "tool_local" }))} onClick={(event) => event.stopPropagation()} className="rounded border border-input bg-background px-1 py-0.5 text-[10px]"><option value="project_then_global">project → global</option><option value="global">global only</option><option value="project">project only</option><option value="tool_local">tool local</option></select>}
                </label>
              ))}
        ...
        <div className="space-y-4">
            ...
            <div className="mt-3 space-y-2">
              {store.blueprints.map((blueprint) => (
                <article key={blueprint.id} className="rounded-md border border-border p-3">
                  <div className="flex gap-3"><div className="min-w-0 flex-1"><h3 className="text-sm font-medium">{blueprint.name}</h3><p className="mt-1 text-xs text-muted-foreground">{blueprint.description || "No purpose recorded."}</p><p className="mt-2 text-xs text-muted-foreground">{blueprint.members.map((member) => `${member.skill_id} (${member.scope_policy.replace(/_/g, " ")})`).join(", ")}</p></div>
          ...
          <div className="rounded-lg border border-border bg-card p-4">
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-5"><label className="text-xs font-medium">Project<select value={assignmentProjectId} onChange={(event) => { setAssignmentProjectId(event.target.value); setEffectiveSet(null); }} className="mt-1 w-full rounded-md border border-input bg-background px-2 py-2 text-sm"><option value="">Global / no project</option>{(config?.projects ?? []).map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select></label><label className="text-xs font-medium">Role<select value={assignmentRole} onChange={(event) => setAssignmentRole(event.target.value as SkillSetAssignment["role"])} className="mt-1 w-full rounded-md border border-input bg-background px-2 py-2 text-sm"><option value="default">Default baseline</option><option value="recommended">Recommended candidate</option><option value="work_scope_overlay">Work-scope overlay</option></select></label><label className="text-xs font-medium">Work scope tags<input value={assignmentScope} onChange={(event) => { setAssignmentScope(event.target.value); setEffectiveSet(null); }} className="mt-1 w-full rounded-md border border-input bg-background px-2 py-2 text-sm" placeholder="api, review (optional)" /></label><label className="text-xs font-medium">Priority<input type="number" value={assignmentPriority} onChange={(event) => setAssignmentPriority(Number(event.target.value) || 0)} className="mt-1 w-full rounded-md border border-input bg-background px-2 py-2 text-sm" /></label><label className="text-xs font-medium">Tool providers<input value={providerIds} onChange={(event) => setProviderIds(event.target.value)} className="mt-1 w-full rounded-md border border-input bg-background px-2 py-2 text-sm" placeholder="codex, claude-code" /></label></div>
            <p className="mt-2 text-xs text-muted-foreground">Separate tags with commas. An overlay applies when every tag is present in the current work context; an empty tag list applies to all work scopes. Defaults apply to every scope. Recommendations are candidates only; assign the release as a default or overlay before applying.</p>
            <button type="button" onClick={resolveEffectiveSet} disabled={busy} className="mt-2 rounded border border-border px-2 py-1 text-xs">Resolve active set</button>
            <div className="mt-3 space-y-2">
              {store.releases.map((release) => (
                <article key={release.id} className="rounded-md border border-border p-3"><div className="flex flex-wrap items-start justify-between gap-2"><div><h3 className="text-sm font-medium">{release.blueprint_name} <span className="text-muted-foreground">{release.label}</span></h3><p className="mt-1 text-xs text-muted-foreground">{release.members.length} members · digest {release.content_digest.slice(0, 12)}</p>{release.release_notes && <p className="mt-1 text-xs text-muted-foreground">{release.release_notes}</p>}{release.member_snapshots.length > 0 && <><p className="mt-1 text-[11px] text-muted-foreground">{release.member_snapshots.filter((item) => item.contract_status === "managed").length}/{release.member_snapshots.length} managed contracts frozen</p><div className="mt-2 space-y-1 text-[11px] text-muted-foreground">{release.member_snapshots.filter((item) => item.contract).map((item) => <p key={item.skill_id}><span className="font-medium text-foreground">{item.skill_id}</span>: {item.contract?.purpose.summary} · environments {item.contract?.requirements.runtimes.join(", ") || "none"} · verification {item.contract?.requirements.verification.join(", ") || "none"}</p>)}</div></>}</div><div className="flex gap-2"><button type="button" onClick={() => loadEvaluationSummary(release.id)} disabled={busy} className="rounded border border-border px-2 py-1 text-xs">Evaluations</button><button type="button" onClick={() => loadActivationHistory(release.id)} disabled={busy} className="rounded border border-border px-2 py-1 text-xs">History</button><button type="button" onClick={() => assignRelease(release)} disabled={busy || (assignmentRole !== "recommended" && !providerIds.trim())} className="rounded border border-border px-2 py-1 text-xs">Assign</button></div></div></article>
              ))}
            ...
            {store.assignments.length > 0 && <div className="mt-4 border-t border-border pt-3"><h3 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Assignments</h3><div className="mt-2 space-y-2">{store.assignments.map((assignment) => { const release = assignmentRelease(assignment); const project = (config?.projects ?? []).find((item) => item.id === assignment.project_id); return <div key={assignment.id} className="flex items-center gap-3 rounded-md bg-muted/40 px-3 py-2 text-xs"><span className="min-w-0 flex-1 truncate">{release?.blueprint_name ?? "Unknown release"} → {project?.name ?? "Global"} / {assignment.role === "default" ? "Default baseline" : assignment.role === "recommended" ? "Recommended candidate (not applied)" : `Overlay: ${describeWorkScopeTags(assignment)}`}</span><button type="button" onClick={() => previewActivation(assignment.id)} disabled={busy || !assignment.active || assignment.role === "recommended"} className="rounded border border-border px-2 py-1">Preview</button><button type="button" onClick={() => inspectDrift(assignment.id)} disabled={busy || !assignment.active || assignment.role === "recommended"} className="rounded border border-border px-2 py-1">Drift</button><button type="button" onClick={() => applyActivation(assignment.id)} disabled={busy || !assignment.active || assignment.role === "recommended"} className="rounded border border-primary/50 px-2 py-1 text-primary">Apply</button><button type="button" onClick={() => void updateStore(() => invoke<SkillSetStore>("set_skill_set_assignment_active", { request: { assignment_id: assignment.id, active: !assignment.active } }))} className="rounded border border-border px-2 py-1">{assignment.active ? (assignment.role === "recommended" ? "Candidate" : "Active") : "Inactive"}</button><button type="button" onClick={() => void updateStore(() => invoke<SkillSetStore>("delete_skill_set_assignment", { assignmentId: assignment.id }))} className="text-muted-foreground"><Trash2 size={14} /></button></div>; })}</div></div>}
            {plan && <div className="mt-4 rounded-md border border-border bg-muted/30 p-3 text-xs"><div className="flex justify-between"><strong>Activation preview: {describeWorkScopeTags(plan)}</strong><span>{plan.operations.filter((item) => item.action === "enable").length} enable · {plan.operations.filter((item) => item.action === "unchanged").length} unchanged</span></div>{plan.requires_shared_root_confirmation && <p className="mt-2 text-warning">Confirmation required — shared root affects: {formatSkillBindingImpacts(plan.shared_impacts, ", ")}</p>}{plan.missing_skill_ids.length > 0 && <p className="mt-2 text-destructive">Missing: {plan.missing_skill_ids.join(", ")}</p>}<div className="mt-2 space-y-1">{plan.operations.map((operation) => <p key={`${operation.skill_instance_id}:${operation.tool_id}`}>{operation.action} {operation.skill_id} → {operation.tool_id} <span className="text-muted-foreground">({operation.reason})</span></p>)}</div></div>}
            {effectiveSet && <div className="mt-3 rounded-md border border-border bg-muted/30 p-3 text-xs"><div className="flex justify-between"><strong>Effective set: {describeWorkScopeTags(effectiveSet)}</strong><span>{effectiveSet.release_ids.length} releases · {effectiveSet.members.length} skills</span></div>{effectiveSet.unresolved_skill_ids.length > 0 && <p className="mt-2 text-destructive">Preview blocked — unresolved: {effectiveSet.unresolved_skill_ids.join(", ")}</p>}<div className="mt-2 space-y-1">{effectiveSet.members.map((member) => <p key={`${member.skill_id}:${member.scope_policy}`}>{member.skill_id} <span className="text-muted-foreground">[{member.scope_policy.replace(/_/g, " ")}] ← {member.included_by_release_ids.length} release(s){member.skill_instance_id ? ` · ${member.skill_instance_id}` : " · unresolved"}</span></p>)}</div></div>}
            {drift && <div className="mt-3 rounded-md border border-border bg-muted/30 p-3 text-xs"><div className="flex justify-between"><strong>Binding drift: {describeWorkScopeTags(drift)}</strong><span className={drift.compliant ? "text-primary" : "text-destructive"}>{drift.compliant ? "Compliant" : "Action needed"}</span></div>{drift.missing_skill_ids.length > 0 && <p className="mt-2 text-destructive">Missing: {drift.missing_skill_ids.join(", ")}</p>}{drift.disabled_operations.map((operation) => <p key={`${operation.skill_instance_id}:${operation.tool_id}`} className="mt-1">Enable {operation.skill_id} → {operation.tool_id}</p>)}{drift.compliant && <p className="mt-2 text-muted-foreground">All required provider bindings are active.</p>}</div>}
            ...
            {evaluationSummary && <div className="mt-3 rounded-md border border-border bg-muted/30 p-3 text-xs"><strong>Evaluation summary · <span className={evaluationSummary.is_verified ? "text-primary" : "text-warning"}>{evaluationSummary.is_verified ? "Verified" : "Not verified"}</span></strong><p className="mt-1">{evaluationSummary.total_count} recorded · passed {evaluationSummary.passed_count} · failed {evaluationSummary.failed_count} · blocked {evaluationSummary.blocked_count}</p><p className="mt-1 text-muted-foreground">Frozen cases passed: {evaluationSummary.verified_case_count}/{evaluationSummary.required_case_count}. Only evidence-backed evaluation records are counted.</p></div>}
          </div>
        </div>
      </section>
    </div>
