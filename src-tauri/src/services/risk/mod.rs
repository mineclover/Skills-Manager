...
use crate::models::{LlmProvider, RiskScanMode, Skill, SkillRiskReport};
...
pub async fn scan_skill(
    ...
    let key = cache::RiskCache::build_key(&skill.instance_id, path, mtime, SCANNER_VERSION, mode);
    ...
    let blocks: Vec<parser::CodeBlock> = files.into_iter().flat_map(|f| f.blocks).collect();
    ...
    if mode.is_deep() {
        if let Some(provider) = llm_provider {
            ...
            if should_trigger_llm {
                ...
                let llm_timeout =
                    std::time::Duration::from_secs(provider.timeout_secs.unwrap_or(60) as u64);
...
mod tests {
    ...
    async fn scan_skill_basic_detects_rm_rf_root() {
        ...
        assert!(report
            .findings
            .iter()
            .any(|f| f.rule_id == "destructive.rm_rf_root"));
    }
    ...
    async fn scan_all_skills_aggregates_reports() {
        ...
        let evil = reports
            .iter()
            .find(|r| r.instance_id.ends_with("evil_multi_004"))
            .unwrap();
        assert_eq!(evil.level, crate::models::RiskLevel::Critical);
        let safe = reports
            .iter()
            .find(|r| r.instance_id.ends_with("safe_multi_005"))
            .unwrap();
