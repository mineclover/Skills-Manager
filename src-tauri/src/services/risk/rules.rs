...
impl Rule {
    fn compile(&self) -> RuleCompiled {
        RuleCompiled {
            ...
            regex: Regex::new(&format!("(?i){}", self.pattern)).expect("rule regex must compile"),
...
mod tests {
    ...
    fn rule_curl_simple_api_no_pipe_not_critical() {
        ...
        assert!(
            findings
                .iter()
                .all(|f| f.rule_id != "network.pipe_to_shell"),
            "不应命中 pipe_to_shell"
        );
    }
    ...
    fn rule_sudo_apt_skipped() {
        ...
        assert!(findings.is_empty(), "sudo apt install 应被排除");
    }
    ...
    fn rule_python_os_import_high() {
        ...
        assert!(findings
            .iter()
            .any(|f| f.rule_id == "payload.python_os_import"));
    }
    ...
    fn context_code_example_downgrades() {
        ...
        assert!(findings.is_empty(), "代码示例块内的 rm -rf 应降权到不报告");
    }
